...
?>
    <main x-data="{showDeleteModal: false, deleteId: null, showUpdateModal: false, editData: null}"
        ...
        <div class="max-w-4xl mx-auto rounded-lg bg-white h-fit shadow-md rounded min-w-1/4">
            <div class="p-4 w-full bg-black flex gap-x-2 items-center">
                ...
                <svg fill="#ffffff" height="16px" width="16px" version="1.1" id="Capa_1"
                     ...
                     viewBox="0 0 488.4 488.4" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            ...
            <!-- Form as a vertical stack of rows, each with 3 columns -->
            <form class="p-6 space-y-3 mb-2 w-full b">
                <!-- Row: IATA -->
                <div class="grid grid-cols-[20%_50%_20%] gap-4 items-center">
                    <p class="text-sm font-bold text-black">IATA</p>
                ...
                <!-- Row: ICAO -->
                <div class="grid grid-cols-[20%_50%_20%] gap-4 items-center">
                    <p class="text-sm font-bold text-black">ICAO</p>
                    <input type="text" class="w-full border border-gray-300 rounded px-3 py-2"/>
                    <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"/>
                </div>
                ...
                <!-- Row: Airport Name -->
                <div class="grid grid-cols-[20%_50%_20%] gap-4 items-center">
                    <p class="text-sm font-bold text-black">Airport Name</p>
                    <input type="text" class="w-full border border-gray-300 rounded px-3 py-2"/>
                    <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"/>
                </div>
                ...
                <!-- Row: Location -->
                <div class="grid grid-cols-[20%_50%_20%] gap-4 items-center">
                    <p class="text-sm font-bold text-black">Location (City)</p>
                    <input type="text" class="w-full border border-gray-300 rounded px-3 py-2"/>
                    <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"/>
                </div>
                ...
                <!-- Row: DST -->
                <div class="grid grid-cols-[20%_50%_20%] gap-4 items-center">
                    <p class="text-sm font-bold text-black">DST</p>
                    <input type="text" class="w-full border border-gray-300 rounded px-3 py-2"/>
                    <input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600"/>
                </div>
            </form>
        ...
        <div class="overflow-hidden w-full h-fit overflow-x-auto rounded-sm border border-neutral-300 dark:border-neutral-700">
            <table class="w-full font-medium text-left text-sm text-black dark:text-black">
                ...
                <?php foreach ($airlines as $airline) : ?>
                    <tr class="even:bg-black/5 dark:even:bg-white/10">
                        <td class="p-4"><?= $airline['id'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['iata'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['icao'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['airline'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['callsign'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['country'] ?? 'N/A' ?></td>
                        <td class="p-4"><?= $airline['comments'] ?? 'N/A' ?></td>
                        <td>
