<div x-cloak x-show="showUpdateModal" x-transition.opacity.duration.200ms
    ...
    <!-- Modal Dialog -->
    <form action="/aircraft" method="POST" x-show="showUpdateModal"
        ...
        <div class="flex items-center justify-between border-b border-neutral-300 bg-neutral-50/60 p-4 dark:border-neutral-700 dark:bg-neutral-950/20">
            <h3 id="defaultModalTitle" class="font-semibold tracking-wide text-neutral-900 dark:text-white" x-text="editData ? 'Edit Aircraft ' + editData.id : 'Edit Aircraft'"></h3>
        ...
        <div class="px-4 py-8 w-full">
            <div class="w-full">
                ...
                <div class="flex w-full flex-col gap-1 text-white">
                    <label for="model" class="w-fit pl-0.5 text-sm text-white">MODEL</label>
                    <input id="model" type="text" :value="editData ? editData.model ?? '' : ''" class="w-full rounded-sm border border-neutral-300 min-w-92 font-medium bg-neutral-50 p-3 text-white text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black disabled:cursor-not-allowed disabled:opacity-75 dark:border-neutral-700 dark:bg-neutral-900/50 dark:focus-visible:outline-white" name="model" placeholder="" autocomplete="name" />
                </div>
            </div>
        </div>
        <!-- Dialog Footer -->
