...
?>
    <main x-data="{showDeleteModal: false, deleteId: null, showUpdateModal: false, editData: null}"
        ...
        <div class="max-w-4xl mx-auto rounded-lg bg-white h-fit shadow-md rounded min-w-1/4">
            <div class="p-4 w-full bg-blue-900 flex gap-x-2 items-center">
        ...
        <!-- Main Table -->
        <div class="overflow-hidden w-full h-fit overflow-x-auto rounded-sm border border-blue-300 dark:border-blue-700">
            <table id="airlineUsersTable" class="w-full font-medium text-left text-sm text-black dark:text-black">
                ...
                <?php foreach ($airline_users as $airline_user) : ?>
                    <tr class="even:bg-blue-900/5 dark:even:bg-white/10">
                        ...
                        <td class="p-4"
                            data-field="password"><?= str_repeat('*', min(8, strlen($airline_user['password'] ?? ''))) ?></td>
                        ...
                        <?php if (!\Core\Session::role(['user', 'staff'])) : ?>
                            <td>
                                ...
                                <button @click="showDeleteModal=true; deleteId = <?= $airline_user['id'] ?>"
                                        type="button"
